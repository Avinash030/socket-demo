...
$.ajax({url: "https://www.tradedesk.zecoex.com/currency_api/trade_market_price.php", success: function(result){
      ...
      //console.log(obj);
        for (var i = 0; i <= obj.length; i++) {
          ...
          document.getElementById(obj[i].currency_symbol+"_low24hr_trade"+obj[i].fiat_id).innerHTML = Number(obj[i].low24hr).toFixed(8);
          document.getElementById(obj[i].currency_symbol+"_high24hr_trade"+obj[i].fiat_id).innerHTML = Number(obj[i].high24hr).toFixed(8);
          document.getElementById(obj[i].currency_symbol+"_quoteVolume_trade"+obj[i].fiat_id).innerHTML = Number(obj[i].quoteVolume).toFixed(8);
        }
    }
});
